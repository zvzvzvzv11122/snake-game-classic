...
export const CaseCard: React.FC<CaseCardProps> = ({ caseData, onOpen, disabled }) => {
  // Get up to 4 unique sample images from the loot table
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    // Set to a generic placeholder (40x40 light gray square)
    e.currentTarget.src = "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNDAiIGhlaWdodD0iNDAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PHJlY3Qgd2lkdGg9IjQwIiBoZWlnaHQ9IjQwIiBmaWxsPSIjZTB expresiónRegularMjNlMCIvPjwvc3ZnPg=="; // Light gray placeholder
    e.currentTarget.onerror = null; // Prevent infinite loop if placeholder itself somehow fails
  };
  ...
  return (
    <div className={`bg-slate-800 rounded-xl shadow-lg overflow-hidden transition-all duration-300 transform hover:scale-105 hover:shadow-2xl flex flex-col ${disabled ? 'opacity-60' : ''}`}>
      <img 
        ...
        onError={handleImageError} // Also apply to case image if needed, though less likely problem here
      />
            ...
            <div className="flex flex-wrap gap-2">
              {sampleLootImages.map((imageUrl, index) => (
                <img
                  ...
                  className="w-10 h-10 object-cover rounded border border-slate-700 bg-slate-700" // Added bg-slate-700 for better placeholder appearance
